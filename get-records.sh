...
# timestamp, station_name, lat, lon, standard_bikes, e_bikes, nb_docks, free_docks 
TIMESTAMP=$(cat $DL_FILE | jq '.records[0].record_timestamp')
NB_RECORDS=$(cat $DL_FILE | jq '.records | length')
cat $DL_FILE | jq -r '.records[] as $r | [$r.record_timestamp,$r.fields.station_name,$r.geometry.coordinates[0],$r.geometry.coordinates[1],$r.fields.nbbike,$r.fields.nbebike,$r.fields.nbedock,$r.fields.nbfreeedock] | @csv' >> ${DL_DIR}/$(date +"%Y%m%d")-velib-records.csv
